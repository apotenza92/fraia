...
- `.github/workflows/ci.yml` is manual-only. Its deterministic checks always run when dispatched; its five-target native package preflight is an explicit opt-in that consumes already reviewed runtime payloads and never constructs or downloads CalculiX.
- `.github/workflows/release.yml` is the only desktop release workflow. It is tag-only and packages exactly five reviewed native targets: macOS ARM64/x64, Linux ARM64/x64, and Windows x64. Windows ARM64 is unsupported and must fail closed until Electron, the bundled `fraia-appd` sidecar, and a reviewed bundled CalculiX runtime all pass the native packaged solver test on that target.
- Stable and beta use separate bundle/package identities and static update feeds. Automatic updating is currently enabled only for signed and notarized macOS packages; do not enable Windows or Linux updating until that platform has an equivalent trusted installation boundary.
...
- `stable-release` always requires final human approval. `beta-release` may publish after every required gate passes.
- `MACOS_UPDATER_BOOTSTRAP_TAG` is a one-time exact tag in the channel's `stable-updater-verification` or `beta-updater-verification` environment. Those environments contain no signing credentials and require no publication reviewer. Use the variable only when that channel has no prior public Fraia package, remove it after the bootstrap release, and never advance it to bypass N-1 updater tests.
...
- The source repository is currently private. The maintained same-repository release workflow must fail before building until the owner either makes it public or reviews a separate public-binary repository design that preserves unauthenticated updates and publicly verifiable provenance. Do not weaken this gate or introduce a cross-repository token implicitly.
