...
- `stable-release` and `beta-release` always require final human approval. A stable release is never marked prerelease; a beta release is always marked prerelease and must not become GitHub's latest stable release.
- `MACOS_UPDATER_BOOTSTRAP_TAG` is a one-time exact tag scoped to each channel's `stable-updater-verification` or `beta-updater-verification` environment. Those environments contain no signing credentials and require no publication reviewer. Use the variable only when no prior public package exists for that exact channel, remove it after the bootstrap release, and never advance it to bypass N-1 updater tests.
...
- Stable release jobs must use maintained Fraia icons at `apps/fraia-electron/build/`; beta release jobs must use the maintained beta icons at `apps/fraia-electron/build/beta/`. Never publish either channel with Electron's default icon.
...
- Homebrew casks may be added only through an approved release gate, with distinct stable and beta cask tokens and no cross-channel application or user-data collision. Do not add store publication without a separate approved consumer and release contract.
