...
- Use the official shadcn Base UI components and Nova style configured by `apps/fraia-electron/components.json` for app chrome and standard controls. Compose domain UI from the existing `src/components/ui` primitives instead of recreating buttons, inputs, checkboxes, dialogs, menus, tabs, or tooltips. Follow Base UI's `render` composition API; do not introduce Radix dependencies, `asChild`, Radix CSS variables, or Radix state selectors.
- Treat `apps/fraia-electron/src/components/ui` as registry-generated source: refresh it only through the project-pinned shadcn CLI and never hand-edit it. Generated controls may receive application `className` values for layout, sizing, placement, truncation, or scrolling only; their colours, borders, radii, shadows, typography, icon sizing, and interaction states remain registry-owned.
- Isolate any reviewed standard-control exception in `apps/fraia-electron/src/components/domain-ui`. Each exception must document the missing official behavior, compose official controls where possible, expose a testable marker, be explicitly allowlisted by `scripts/check-shadcn-contract.cjs`, and have deterministic accessibility coverage.
- Keep document tabs on the adopted full-width Nova document bar: semantic background, one full-width bottom divider, 8 px inset and gaps, 32 px tabs/actions, default rather than line tabs, muted selected surface with no underline, and document actions immediately after the newest tab. Tab-bar actions use the standard Nova outline border with a resting fill that matches the bar; the compact open action uses a plus icon with an explicit accessible label and tooltip, while new blank models use a separate non-split action. Closable tabs and their focus/reorder behavior remain a reviewed domain composition.
- Run `npm run audit:shadcn` from `apps/fraia-electron` for a human-invoked, network-dependent comparison with the live registry. Keep this read-only audit separate from deterministic `npm run check`; review upstream diffs and apply updates manually through the CLI. Do not add scheduled refreshes, automatic overwrites, update bots, or auto-merge.
...
- Release tags must be pushed by the repository owner account `apotenza92`. That owner tag push is the final publication authorization, so Stable and Beta publication must not request a second deployment review from the same account. A stable tag that advances both feeds publishes both identity-specific package sets after they pass. A stable release is never marked prerelease; a beta release is always marked prerelease and must not become GitHub's latest stable release.
...
- Every approved release stages the checksum-sealed `homebrew-publication.tar.gz` in the exact draft asset contract before publication, attests it with the release assets, validates both architectures through native Homebrew installs, and then dispatches the central tap workflow with the protected Homebrew Dispatcher App. Source workflows must never write the tap repository directly. The tap remains the sole cask writer and reconciliation authority.
