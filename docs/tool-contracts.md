...
Fraia 0.0.3 exposes one public connection: **Sign in with ChatGPT** through Pi's `openai-codex` OAuth flow. It does not expose provider search, API-key entry, model selection, or reasoning selection. The ChatGPT authorization is stored as an Electron `safeStorage` encrypted blob beneath Electron user data. If operating-system encryption is unavailable, Fraia refuses to persist the connection rather than falling back to plaintext. Projects, chat records, logs, and diagnostics never contain provider credentials. Pi sessions and duplicate Pi transcripts are not persisted.
