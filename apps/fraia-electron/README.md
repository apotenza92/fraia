...
Every packaged platform checks only the static feed embedded for its identity. Stable checks `stable/<platform>/<arch>` and rejects prereleases; beta checks `beta/<platform>/<arch>` and accepts the separately packaged beta line. Publications replace only the matching channel path. The Fraia menu exposes a Base UI update surface with manual-check results, automatic-check frequency, last successful check, download percentage/size/speed/estimated time, release notes, retryable user-safe errors, and a persistent restart action. New stable users default to weekly checks, while new beta users default to daily checks; a saved user choice always wins. Existing single-timestamp updater settings migrate silently to separate last-attempt and last-successful-check fields, and failed automatic checks use bounded retry backoff rather than delaying the next attempt for a full interval.
...
Fraia is published through the `apotenza92/tap` Homebrew tap as the `fraia` and `fraia@beta` casks. Both declare `auto_updates true` and install the exact signed `Fraia.app` or `Fraia Beta.app` identity package. Trusted tap automation checks approved Fraia releases every six hours and updates a cask only after verifying the public digest, version, bundle identity, architecture, Developer ID signature, hardened runtime, notarization, stapling, and Gatekeeper acceptance. Install with `brew install --cask apotenza92/tap/fraia` or `brew install --cask apotenza92/tap/fraia@beta`.

Homebrew remains an optional installation and update route: it does not disable or redirect Fraia's embedded updater, and the app never invokes `brew` to check or install an application update. A user may therefore receive a newer same-channel app either from Fraia's authenticated feed or through `brew upgrade`; both paths preserve the identity-specific user-data directory. The casks never project one channel's package into the other cask.
