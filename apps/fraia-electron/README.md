...
Signed macOS packages check their channel-specific static feed automatically, download in the background, and install on quit. The native application menu offers Never, On Startup, Hourly, Every 6 Hours, Every 12 Hours, Daily, and Weekly frequencies plus a manual check. Stable defaults to Daily, beta defaults to Every 6 Hours, and the choice persists in the isolated channel user-data directory. Release CI tests previous-to-current valid, corrupt, and wrong-signature paths on native ARM64 and Intel runners. The exact first release in each channel requires the one-time `MACOS_UPDATER_BOOTSTRAP_TAG`; subsequent releases require real N-1 evidence. Windows and Linux updating stays disabled until those platforms have a trusted signing boundary.
