...
There is no Homebrew or store publication yet. An approved Homebrew rollout must use the tap's established `fraia` and `fraia@beta` cask tokens, declare `auto_updates true`, and install the exact signed `Fraia.app` or `Fraia Beta.app` identity package. Homebrew remains an optional installation and update route: it must not disable or redirect Fraia's embedded updater, and the app never invokes `brew` to check or install an application update. A user may therefore receive a newer same-channel app either from Fraia's authenticated feed or through `brew upgrade`; both paths preserve the identity-specific user-data directory. The casks must not project one channel's package into the other cask.
