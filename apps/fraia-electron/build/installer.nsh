...
!macro customCheckAppRunning
  ...
  ${if} $R0 == 0
    ...
    Quit
  ${endIf}
  nsExec::Exec `"$SYSDIR\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "& { $$installDirectory = [Environment]::GetEnvironmentVariable('FRAIA_NSIS_INSTALL_DIR'); if ([string]::IsNullOrWhiteSpace($$installDirectory)) { exit 1 }; $$installPrefix = [IO.Path]::GetFullPath($$installDirectory).TrimEnd('\') + '\'; $$deadline = (Get-Date).AddSeconds(15); do { $$processes = @(Get-CimInstance -ClassName Win32_Process | Where-Object { $$_.ExecutablePath -and $$_.ExecutablePath.StartsWith($$installPrefix, [System.StringComparison]::OrdinalIgnoreCase) -and $$_.ProcessId -ne $pid }); if ($$processes.Count -eq 0) { exit 0 }; Start-Sleep -Milliseconds 250 } while ((Get-Date) -lt $$deadline); exit 1 }"`
  Pop $R0
